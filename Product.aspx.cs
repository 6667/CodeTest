...
namespace FrameWork.web.Manager.Module.AspTest.CodeTest
{
    public partial class Product : System.Web.UI.Page
    {
        ...
        private void BindData()
        {
            ...
            List<Product_Info> lst = new List<Product_Info>();
			
            //catchKeyWord这个对象只是用来存放查询条件的信息，你完全可以用两个string变量存储查询条件，但是
            //采用这种方法比较方便也便于以后的开发
            Search_Info catchKeyWord = new Search_Info();
            string buffer = CustomerID.Text.ToString();
            catchKeyWord.intCustomerID = Convert.ToInt32(buffer);
            catchKeyWord.strStart = TextBox_dtStart.Text.ToString();
            catchKeyWord.strEnd = TextBox_dtEnd.Text.ToString();
            //Keywords_Table_Search这个函数就是在进行数据的筛选
            lst = BusinessFacade_sample.Buying_Record_Search(AspNetPager1.PageSize, AspNetPager1.CurrentPageIndex, catchKeyWord, out RecordCount);
